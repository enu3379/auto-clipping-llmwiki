...
- macOS에서 주로 검증했다. Windows는 local exe/portable zip 흐름을 우선 사용한다.
- Node.js 20+
...
Windows에서 먼저 확인할 때는 installer/MSI/NSIS 번들링을 피하고 local exe만 빌드한다.
`npm run tauri build`는 Windows에서 installer bundler까지 실행되어 환경에 따라 실패할 수 있다.
...
Windows portable zip이 필요하면 local app 빌드 후 아래를 실행한다.

```powershell
npm run package:windows:portable
```

산출물은 `dist-portable/LLM-Wiki-<version>-windows-x64-portable.zip`에 생긴다. 이 zip은
`LLM Wiki.exe`, `pdfium/`, `mcp-server/`를 함께 담으므로 압축을 푼 뒤 같은 폴더 구조를 유지해서
`LLM Wiki.exe`를 실행한다.
...
```text
%APPDATA%\com.llmwiki.app\app-state.json
```
